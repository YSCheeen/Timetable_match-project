...
def read_student_excel(filename):
    ...
    with open('学生数据.json', mode='w', encoding='utf-8') as f:
        json.dump(slots, f)

    print('文件-' + '学生数据.json 已生成')


def read_teacher_excel(filename):
    data = xlrd.open_workbook(filename)
    table = data.sheets()[0]
    ...
    with open('老师数据.json', mode='w', encoding='utf-8') as f:
        json.dump(slots, f)

    print('文件-' + '老师数据.json 已生成')
